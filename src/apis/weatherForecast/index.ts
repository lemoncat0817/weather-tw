...
export const getThirtySixHoursWeatherForecast = () =>
  request.get<any, thirtySixHoursWeatherData>(API.GETTHIRTYSIXHOURSWEATHERFORECAST_URL)
// 獲取未來三天氣數據
export const getThreeDaysWeatherForecast = () =>
  request.get<any, threeDaysWeatherData>(API.GETTHREEDAYSWEATHERFORECAST_URL)
// 獲取未來一周天氣數據
export const getWeekWeatherForecast = () =>
  request.get<any, weekWeatherData>(API.GETWEEKWEATHERFORECAST_URL)
// 獲取全臺灣各鄉鎮市區預報資料
export const getAllTaiwanWeatherForecast = (locationId: string) =>
  request.get<any, allTaiwanWeatherData>(
    API.GETALLTAIWANWEATHERFORECAST_URL + `?locationId=${locationId}`
  )
// 獲取全臺灣各鄉鎮市區一週預報資料
export const getAllTaiwanWeekWeatherForecast = (locationId: string) =>
  request.get<any, allTaiwanWeekWeatherData>(API.GETALLTAIWANWEEKWEATHERFORECAST_URL + locationId)
