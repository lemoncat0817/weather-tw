<template>
  <div>
    ...
    <el-card style=" margin: 40px 50px; border-radius: 20px; ">
      <el-tag class="tag">
        <div class="title" style="color:salmon; font-weight: bold;">全臺灣各鄉鎮市區(一週)預報 - {{ location }}</div>
        ...
        <div class="rwdDiv2">資料內容包含(天氣、降雨機率、溫度、平均溫度、</div>
        <div class="rwdDiv2">體感溫度、平均相對濕度、最小及最大舒適度指數)</div>
        <div class="rwdDiv2">、最大風速、風向，平均露點溫度)</div>
        <div>
              ...
              <el-dropdown-menu style="padding: 0; ">
                ...
                <el-card class="card" style="width: 100%; background: pink; ">
                  <div class="northMid">
                    <div class="north">
                      <div>北部:</div>
                      <el-radio-group v-model="allTaiwanWeekStore.locationId">
                        <el-radio style="margin: 0px 3px; color:brown;" value="F-D0047-063" size="large">臺北市</el-radio>
                        <el-radio style="margin: 0px 3px; color:brown;" value="F-D0047-071" size="large">新北市</el-radio>
                        <el-radio style="margin: 0px 3px; color:brown;" value="F-D0047-051" size="large">基隆市</el-radio>
                        <el-radio style="margin: 0px 3px; color:brown;" value="F-D0047-055" size="large">新竹市</el-radio>
                        <el-radio style="margin: 0px 3px; color:brown;" value="F-D0047-007" size="large">桃園市</el-radio>
                        <el-radio style="margin: 0px 3px; color:brown;" value="F-D0047-011" size="large">新竹縣</el-radio>
                        <el-radio style="margin: 0px 3px; color:brown;" value="F-D0047-003" size="large">宜蘭縣</el-radio>
                      </el-radio-group>
                    </div>
                    <div class="mid">
                      <div>中部:</div>
                      <el-radio-group v-model="allTaiwanWeekStore.locationId">
                        <el-radio style="margin: 0px 3px; color:brown;" value="F-D0047-075" size="large">臺中市</el-radio>
                        <el-radio style="margin: 0px 3px; color:brown;" value="F-D0047-015" size="large">苗栗縣</el-radio>
                        <el-radio style="margin: 0px 3px; color:brown;" value="F-D0047-019" size="large">彰化縣</el-radio>
                        <el-radio style="margin: 0px 3px; color:brown;" value="F-D0047-023" size="large">南投縣</el-radio>
                        <el-radio style="margin: 0px 3px; color:brown;" value="F-D0047-027" size="large">雲林縣</el-radio>
                      </el-radio-group>
                    </div>
                  </div>
                  <div class="southEastOut">
                    <div class="south">
                      <div>南部:</div>
                      <el-radio-group v-model="allTaiwanWeekStore.locationId">
                        <el-radio style="margin: 0px 3px; color: brown;" value="F-D0047-067" size="large">高雄市</el-radio>
                        <el-radio style="margin: 0px 3px; color: brown;" value="F-D0047-079" size="large">臺南市</el-radio>
                        <el-radio style="margin: 0px 3px; color: brown;" value="F-D0047-059" size="large">嘉義市</el-radio>
                        <el-radio style="margin: 0px 3px; color: brown;" value="F-D0047-031" size="large">嘉義縣</el-radio>
                        <el-radio style="margin: 0px 3px; color: brown;" value="F-D0047-035" size="large">屏東縣</el-radio>
                      </el-radio-group>
                    </div>
                    <div class="east">
                      <div>東部:</div>
                      <el-radio-group v-model="allTaiwanWeekStore.locationId">
                        <el-radio style="margin: 0px 3px; color: brown;" value="F-D0047-043" size="large">花蓮縣</el-radio>
                        <el-radio style="margin: 0px 3px; color: brown;" value="F-D0047-039" size="large">臺東縣</el-radio>
                      </el-radio-group>
                    </div>
                    <div class="out">
                      <div>離島:</div>
                      <el-radio-group v-model="allTaiwanWeekStore.locationId">
                        <el-radio style="margin: 0px 3px; color: brown;" value="F-D0047-087" size="large">金門縣</el-radio>
                        <el-radio style="margin: 0px 3px; color: brown;" value="F-D0047-083" size="large">連江縣</el-radio>
                        <el-radio style="margin: 0px 3px; color: brown;" value="F-D0047-047" size="large">澎湖縣</el-radio>
                      </el-radio-group>
                    </div>
                    <div class="search">
                      <div>搜尋鄉鎮市區:</div>
                      <div class="districtSearch"> <el-input v-model="allTaiwanWeekStore.districtKeyword"
      ...
      <el-table :data="searchWeather" border max-height="calc(100vh - 385px)" style="width: 90%; margin: 20px auto; "
        v-loading="loading">
        <!-- 鄉鎮名稱 -->
        <el-table-column fixed="left" align="center" width="105" label="鄉鎮名稱">
          <template #default="{ row }">
            ...
            <el-popover placement="right" :width="150" trigger="click">
              ...
              <template #default>
                <div @click="goAllTaiwan(row.locationName)" class="locationName" style="color:gray; font-size: 12px;">
                  查看三天內預報</div>
              </template>
            </el-popover>
          </template>
          ...
          <el-table-column align="center" width="425">
            <template #header>
              <div class="text-center"
                v-if="allTaiwanWeekWeather.length > 0 && allTaiwanWeekWeather[0].weatherElement[1].time[item - 1].startTime">
                <div style="color: palevioletred; font-size: 14px; font-weight: bold; border: 1px solid gray; ">
                  {{ `日期: ${allTaiwanWeekWeather[0].weatherElement[1].time[item - 1].startTime?.slice(5, 7)}
                  /${allTaiwanWeekWeather[0].weatherElement[1].time[item - 1].startTime?.slice(8, 10)}` }}
                </div>
                <div style="font-size: 14px; border: 1px solid gray; color: royalblue; ">
                  {{ `時間: ${allTaiwanWeekWeather[0].weatherElement[1].time[item - 1].startTime?.slice(11, 16)}` }}
                </div>
              </div>
            </template>
            <template #default="{ row }">
...
// 引入接口以及接口type
import { getAllTaiwanWeekWeatherForecast } from '@/apis/weatherForecast/index'
import type { allTaiwanWeekWeatherData, Location } from '@/apis/weatherForecast/type/allTaiwanWeek'
// 引入倉庫並使用
...
// 帶著關鍵字前往鄉鎮市區預報
const goAllTaiwan = (locationName: string) => {
  router.push('/weatherForecast/allTaiwan')
  allTaiwanStore.districtKeyword = locationName
  allTaiwanStore.locationId = allTaiwanWeekStore.locationId.slice(0, 9) + ((parseInt(allTaiwanWeekStore.locationId.slice(9, 11)) - 2) < 10 ? '0' +
    (parseInt(allTaiwanWeekStore.locationId.slice(9, 11)) - 2).toString() : (parseInt(allTaiwanWeekStore.locationId.slice(9,
      11)) - 2).toString())
}
// 監聽縣市Id是否改變
watch(() => allTaiwanWeekStore.locationId, () => {
  reqAllTaiwanWeekWeatherForecast(allTaiwanWeekStore.locationId)
})
// 路由守衛
onBeforeRouteLeave((to, from, next) => {
  allTaiwanWeekStore.districtKeyword = ''
  next()
})
// 接收接口回傳的天氣資料
const allTaiwanWeekWeather = ref<Location[]>([])
// 計算搜尋的天氣資料
const searchWeather = computed(() => {
  return allTaiwanWeekWeather.value.filter((item: any) => {
    if (allTaiwanWeekStore.districtKeyword !== '') {
      return item.locationName.match(allTaiwanWeekStore.districtKeyword)
    } else {
      return item
    }
  })
...
// 獲取當前天氣資料
const reqAllTaiwanWeekWeatherForecast = async (locationId: string) => {
  const res: allTaiwanWeekWeatherData = await getAllTaiwanWeekWeatherForecast(locationId)
  location.value = res.data.records.locations[0].locationsName
  allTaiwanWeekWeather.value = res.data.records.locations[0].location
  loading.value = false
}
// 刷新後渲染當前天氣資料
onMounted(() => {
